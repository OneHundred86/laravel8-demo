...
- 属性值有 expires、path、domain、samesite、httponly、secure等；
  ...
  - samesite:
    ...
    | 值       | 同一站点请求                      | 跨站GET请求（如链接跳转、GET表单提交等） | 跨站非GET请求（如POST表单提交）     | 跨域ajax请求（包括GET、POST）                                           | 适用场景                                                                                                                              | 安全级别 |
    | -------- | --------------------------------- | ---------------------------------------- | ----------------------------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | -------- |
    | `Strict` | Request`能携带`；Response`能修改` | Request`不能携带`；Response`能修改`      | Request`不能携带`；Response`能修改` | Request`不能携带`；Response`不能修改`                                   | 登录、支付等敏感操作                                                                                                                  | 最高     |
    | `Lax`    | Request`能携带`；Response`能修改` | Request`能携带`；Response`能修改`        | Request`不能携带`；Response`能修改` | Request`不能携带`；Response`不能修改`                                   | 大多数普通场景（默认推荐）                                                                                                            | 中       |
    | `None`   | Request`能携带`；Response`能修改` | Request`能携带`；Response`能修改`        | Request`能携带`；Response`能修改`   | Request`能携带`；Response`能修改`<br>（需使用https请求，且secure=true） | **跨域共享 Cookie**（需使用https请求）<br>注意：所在页面可以不是https，且页面域名和请求域名可以毫无关系，即可以不是父子或者兄弟域名） | 最低     |
...
#### 场景一：所有跨域域名都在`同一个父域名`下

1. 用于共享状态的域名的cookie设置`domain={父域名}`，让cookie在所有跨域域名中共享；
2. 用于共享状态的域名的cookie设置`samesite=lax`，提升安全级别；
