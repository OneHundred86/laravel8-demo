...
注意：
- cookie是受当前请求影响，和请求所在的页面无关。
- 请求响应后，cookie是设置在请求所属的域名下，和请求所在的页面无关。
- 在浏览器中，cookie 是以 “同源 + 路径 + 安全属性” 的多层隔离机制隔离的
  - 同源策略，即只有`协议`、`域名`、`端口`完全一致的网页，才能共享 Cookie；
  - 路径（Path）：只有当请求路径 以 Cookie 的 Path 开头 时，才能访问该 Cookie；
  - 安全属性
    - Secure 属性：设置后，仅当请求使用 https:// 请求时，浏览器才会发送该 Cookie； 使用 http:// 请求无法携带该 Cookie；
    - HttpOnly 属性：
      - 浏览器端的 JavaScript 无法访问该 Cookie（document.cookie 无法读取或写入）；
      - 浏览器 http(s):// 请求时可以携带该cookie；
      - 服务器通过 http(s):// 响应头 设置或修改该 Cookie。
- 在跨域场景中，比如在页面 http://debug.local/a/b 下跨域请求 http://a.example.com/，则该跨域请求只能携带和修改 http://a.example.com 域名下的cookie，不会影响到 http://debug.local 下的cookie。
...
- 属性值有 expires、path、domain、samesite、httponly、secure等；
  - expires: 
    - 如果没有设置，缺省就是session（关闭所有窗口时（注意是窗口，而不是标签页）摧毁）。
    - 具体值示例：expires=Fri, 21-Nov-2025 03:50:05 GMT; 
  - path: 设置cookie所属的路径。路径下的请求，才可以使用该cookie。
    - 缺省为当前请求的路径的文件夹路径。（受当前请求影响，和请求所在的页面无关）
    - 比如请求 http://a.example.com/a/b，那缺省值为 /a/（后面的/可省略，即/a）。
    - 比如请求 http://a.example.com/a/b/，那缺省值为 /a/b/（后面的/可省略，即/a/b）。
    - 具体值示例：path=/;
  - domain：
    - 如果没有设置，缺省就是`本域名`。比如请求（即使在其他域请求） http://a.example.com/debug ，那本域名是 a.example.com。（受当前请求影响，和请求所在的页面无关）
    - 如果设置具体值，表示在`本域名`和它的`子域名`下都可以使用该cookie。
    - 具体值示例：domain=a.example.com;
  - samesite:
    - 缺省值为浏览器默认值，比如chrome的默认值为lax。
  
    | 值       | 同站请求<br>（源站点和目标请求需同为http或者https） | 跨站GET请求（如链接跳转、GET表单提交等）<br/>（源站点和目标请求需同为http或者https） | 跨站非GET请求（如POST表单提交）<br/>（源站点和目标请求需同为http或者https） | 适用场景                                                                                                                              | 安全级别 |
    | -------- | --------------------------------------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | -------- |
    | `Strict` | Request`能携带`；Response`能修改`                   | Request`不能携带`；Response`能修改`                                                  | Request`不能携带`；Response`能修改`                                         | 登录、支付等敏感操作                                                                                                                  | 最高     |
    | `Lax`    | Request`能携带`；Response`能修改`                   | Request`能携带`；Response`能修改`                                                    | Request`不能携带`；Response`能修改`                                         | 大多数普通场景（默认推荐）                                                                                                            | 中       |
    | `None`   | Request`能携带`；Response`能修改`                   | Request`能携带`；Response`能修改`                                                    | Request`能携带`；Response`能修改`                                           | **跨域共享 Cookie**（需使用https请求）<br>注意：所在页面可以不是https，且页面域名和请求域名可以毫无关系，即可以不是父子或者兄弟域名） | 最低     |

    **跨域**和**跨站**的区别：
    |      | 跨域                                                       | 跨站                                                                                                                                                            |
    | ---- | ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | 概念 | 指协议、域名、端口任一不同的站点                           | 指`主域名`不同的站点                                                                                                                                            |
    | 例子 | 如 `http://a.example.com` 和 `https://b.example.com`是跨域 | 如 `a.example.com` 和 `b.example.com`属于同一站点（同站），它们的主域名都是`example.com`；<br>`http://a.example.com` 和 `https://b.example.com`也是属于同一站点 |

    注意http和htts混合请求的情况：
      - 在`https页面`（https://b.example.com/）不能加载`http资源`，也不能跨域请求http资源（http://a.example.com/api/cors/debug 和 http://b.example.com/api/cors/debug）。【浏览器原理：**混合内容限制**，避免HTTPS页面安全降级】
      - 在`http页面`（http://b.example.com/）跨域请求`https资源`（https://a.example.com/api/cors/debug），不能携带cookie，也不能修改cookie。【浏览器原理：**Cookie 安全策略**，https请求的cookie资源只能在https作为安全上下文（https页面）中才能被携带和修改】



```text
# Response 响应
# 使用 Set-Cookie 响应头设置cookie，可设置多个。
HTTP/1.1 200 OK
...
Set-Cookie: r=aaaa; expires=Mon, 17-Nov-2025 04:14:30 GMT; Max-Age=7200; path=/; domain=example.com; secure; httponly; samesite=lax
...
### 4.使用cookie共享状态在`cors`（跨域资源共享）的2种应用场景

#### #### 场景一：同一站点，所有跨域域名都在`同一个主域名`下

1. 用于共享状态的域名的cookie原本就支持在所有跨域域名中共享（同站）；
2. 用于共享状态的域名的cookie设置`samesite=lax`或`samesite=strict`，提升安全级别；

#### 场景二：跨域域名之间没有关系

1. 用于共享状态的域名的cookie设置`samesite=none; secure;`（缺点：安全级别较低）
2. 用于共享状态的域名需使用`https`（才能让cookie设置为secure）
...
| ------------ | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **生命周期** | **永久有效**，除非手动删除（通过js代码、浏览器设置或清除缓存）。       | **会话级有效**，仅在当前浏览器标签页关闭前存在。刷新页面数据保留，但关闭标签页后数据丢失。                                                                  |
| **作用域**   | **同源共享**。在同一个协议、域名、端口下的所有标签页和窗口都可以访问。 | **标签页独立**。仅在创建它的那个标签页内有效，即使是同一个网站的其他标签页也无法访问。<br/>注意：是**标签页隔离**，即使是同一个窗口不同标签页数据也不共享。 |
| **存储容量** | 较大，通常为 5MB 左右（因浏览器而异）。                                | 与 `localStorage` 类似，通常也为 5MB 左右。                                                                                                                 |
| **数据共享** | 同一浏览器内，同源的所有页面可以共享 `localStorage` 中的数据。         | 数据完全隔离，不与其他任何标签页（即使是同源的）共享。                                                                                                      |
| **常见用途** | 存储需要长期保留的用户偏好设置、主题、记住登录状态（非敏感信息）等。   | 存储临时会话数据，如表单填写的临时内容、单页应用的状态管理、防止表单重复提交的令牌等。                                                                      |


### 2.js操作

```js
localStorage                        // 获取storage对象
localStorage.setItem('la', 'aaa')   // 设置kv
localStorage.getItem('la')          // 获取k的值
localStorage.removeItem('la')       // 移除k
localStorage.clear()                // 移除全部k

sessionStorage                        // 获取storage对象
sessionStorage.setItem('sa', 'aaa')   // 设置kv
sessionStorage.getItem('sa')          // 获取k的值
sessionStorage.removeItem('sa')       // 移除k
sessionStorage.clear()                // 移除全部k
